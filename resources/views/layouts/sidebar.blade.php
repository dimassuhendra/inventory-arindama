<aside x-cloak
    ...
    <!-- 2. Navigasi Menu (diubah: justify-start agar menumpuk ke atas tanpa spasi melayang) -->
    <nav
        class="flex-1 px-4 py-6 space-y-4 overflow-y-auto custom-scrollbar overflow-x-hidden flex flex-col justify-start">

        <!-- GROUP 1: MAIN MENU -->
        @if (auth()->user()->hasAnyRole(['Super Admin', 'Staff', 'Gudang', 'HRGA']))
            <div x-data="{ open: true }">
                <button @click="open = !open"
                    ...
                    <i class="fa-solid fa-chevron-down text-[9px] transition-transform duration-300"
                        x-show="!isCollapsed" :class="open ? 'rotate-0' : '-rotate-90'"></i>
                </button>
                ...
                <ul x-show="open || isCollapsed" x-collapse class="space-y-1.5 mt-2">
                    ...
                    <li>
                        <a href="{{ route('dashboard') }}" title="Dashboard"
                            ...
                            <span class="ml-3 text-xs font-medium whitespace-nowrap"
                                x-show="!isCollapsed">Dashboard</span>
                        </a>
                    </li>
        ...
        <!-- GROUP 2: INVENTORY CONTROL (Superadmin, Admin, Gudang) -->
        ...
        @if (auth()->user()->hasAnyRole(['Super Admin', 'Staff', 'Gudang', 'HRGA']))
            <div x-data="{ open: true }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-2 py-1.5 text-inv-teal/80 hover:text-inv-mint cursor-pointer">
                    <span class="text-[10px] font-bold uppercase tracking-widest" x-show="!isCollapsed">Master
                        Data</span>
                    <i class="fa-solid fa-chevron-down text-[9px] transition-transform duration-300"
                        x-show="!isCollapsed" :class="open ? 'rotate-0' : '-rotate-90'"></i>
                </button>
        ...
        <!-- GROUP 4: SISTEM (Manajemen Pengguna - Hanya Superadmin) -->
        @role('Super Admin')
            <div x-data="{ open: true }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-2 py-1.5 text-inv-teal/80 hover:text-inv-mint cursor-pointer">
